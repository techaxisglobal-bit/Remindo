...
const transporter = nodemailer.createTransport({
    host: process.env.MICROSOFT_SMTP_HOST || 'smtp.office365.com',
...
const sendOTP = async (email, otp, type = 'signup') => {
    if (!process.env.MICROSOFT_SMTP_USER || !process.env.MICROSOFT_SMTP_PASS) {
        console.warn('MICROSOFT_SMTP_USER or MICROSOFT_SMTP_PASS not set in .env. Falling back to console logging.');
        console.log(`\n\n=== OTP MOCK EMAIL ===\nTo: ${email}\nOTP: ${otp}\nType: ${type}\n======================\n\n`);
        return; 
    }
    ...
    const mailOptions = {
        from: `"Remindo" <${process.env.MICROSOFT_SMTP_FROM || process.env.MICROSOFT_SMTP_USER}>`,
        to: email,
        subject: subject,
        html: `
    ...
    try {
        ...
        console.error('Error sending email via Nodemailer (Office365):', error);
...
const sendInvitation = async (email, task, creatorName, backendUrl) => {
    ...
    if (!process.env.MICROSOFT_SMTP_USER || !process.env.MICROSOFT_SMTP_PASS) {
        console.warn('MICROSOFT_SMTP_USER or MICROSOFT_SMTP_PASS not set. Falling back to console logging for invitation.');
    ...
    try {
        ...
        console.error('Error sending invitation via Nodemailer (Office365):', error);
    }
};
