...
export function MerchantCard({ merchant, isAdminView, onApprove, onReject, onFeature, onEdit, onDelete }: MerchantCardProps) {
    ...
    const statusColors = {
        PENDING: 'bg-yellow-100 text-yellow-800 dark:bg-black dark:text-yellow-500',
        APPROVED: 'bg-green-100 text-green-800 dark:bg-black dark:text-green-500',
        REJECTED: 'bg-red-100 text-red-800 dark:bg-black dark:text-red-500',
        SUSPENDED: 'bg-gray-100 text-gray-800 dark:bg-black dark:text-gray-400',
    };

    return (
        <div className={`relative bg-white dark:bg-black rounded-xl border ${merchant.isFeatured ? 'border-[#e0b596] shadow-sm shadow-[#e0b596]/20' : 'border-gray-200 dark:border-gray-800'} overflow-hidden flex flex-col transition-all hover:shadow-md`}>
            {merchant.isFeatured && (
